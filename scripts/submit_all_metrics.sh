#!/usr/bin/env bash
# Submit one independent Slurm job per experiment for Dice/IoU metric evaluation.
# Usage:  bash scripts/submit_all_metrics.sh
...
for name in "${EXPERIMENTS[@]}"; do
    ...
    job_id=$(EVAL_NAME="${name}" sbatch \
        --job-name="metrics_${name}" \
        --output="slurm_output/medsam2_metrics_${name}_%j.out" \
        scripts/sbatch_medsam2_eval_metrics.sh \
        | awk '{print $NF}')
